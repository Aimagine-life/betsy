...
import { addKnowledge } from "../memory/knowledge.js";
...
export class SkillInstallTool implements Tool {
  name = "skill_install";
  description =
    "Install a skill from GitHub by downloading its SKILL.md and saving it to memory. " +
  ...
  async execute(params: Record<string, unknown>): Promise<ToolResult> {
    ...
    try {
      ...
      // Truncate very large skills to avoid bloating knowledge base
      ...
      addKnowledge({
        ...
        insight: content,
        source: `skillsmp:${githubUrl}`,
      });

      console.log(`skill_install: installed "${skillName}" (${content.length} chars) from ${githubUrl}`);
